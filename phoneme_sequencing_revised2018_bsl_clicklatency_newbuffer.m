...
function phoneme_sequencing_revised2018_bsl_clicklatency_newbuffer(subject, practice)
...
subjectDir = ['data/' subject];
soundDirCVC = 'clickstim/';
soundDirVCV = 'clickstim/';
...
for iB = 1:nBlocks %nBlocks;
    ...
    for i = 1:trialEnd
        if coinToss(i) == 1
            ...
            soundBlockPlay{i}.soundbuffer=PsychPortAudio('OpenSlave', pahandle,1,2);
            s_tmp=[];
            s_tmp=soundValsCVC{trialOrderCVCBlock(counterCVC+1)}.sound;
            soundBlockPlay{i}.sound=s_tmp;
            PsychPortAudio('FillBuffer', soundBlockPlay{i}.soundbuffer, [s_tmp,s_tmp]');
        ...
        elseif coinToss(i) == 0
            ...
            soundBlockPlay{i}.soundbuffer=PsychPortAudio('OpenSlave', pahandle,1,2);
            s_tmp=[];
            s_tmp = soundValsVCV{trialOrderVCVBlock(counterVCV+1)}.sound;
            soundBlockPlay{i}.sound=s_tmp;
            PsychPortAudio('FillBuffer', soundBlockPlay{i}.soundbuffer, [s_tmp,s_tmp]');
    ...
    tone_buffer=PsychPortAudio('OpenSlave', pahandle,1,2);
    tone500 = audioread('/home/coganlab/Psychtoolbox_Scripts/tone500_3.wav');
    PsychPortAudio('FillBuffer', tone_buffer, [0.005*tone500,0.005*tone500]');

    % Start playback pahandle
    ...
    PsychPortAudio('Start', tone_buffer, repetitions, StartCue, WaitForDeviceStart);
    ...
    PsychPortAudio('Stop',tone_buffer);
...
%     postlat = PsychPortAudio('LatencyBias', pahandle);
    ...
    for iTrials = 1:trialEnd
        ...
        PsychPortAudio('Start',soundBlockPlay{iTrials}.soundbuffer,repetitions,tPredictedVisualOnset,0);
        ...
        PsychPortAudio('Stop',soundBlockPlay{iTrials}.soundbuffer)
